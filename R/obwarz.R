...
#' @name obwarz
#' @return obwarz_list returns named list
...
#' @rdname obwarz
#' @export
#' @param o list returned by obwarz_list()
obwarz_dt <- function(o = obwarz_list())
{
    rbindlist(
        l = lapply(
            X = names(o),
            FUN = function(i)
            {
                data.table(JPT_KOD_JE = i, id = o[[i]])
            }
        )
    )
}
